...
\caption{Fitted extrapolations compared to the true likelihood for four different test likelihoods. The live points at 30\% through the run for each example are fitted to a Gaussian, extrapolated to the higher likelihoods that have not yet been explored, and compared to the ground truth. It is clear that truly Gaussian likelihoods are modeled well, while the model deteriorates with increasing non-Gaussianity (seen in the Rosenbrock and Cauchy likelihoods).}
